		...
		h = class e extends Uint8Array {
			...
			toString(t = 'utf8', r = 0, n = this.length) {
				if (
					...
					return '';
				if (t === 'utf8' || t === 'utf-8')
					return yu.decode(this.slice(r, n));
				if (t === 'base64' || t === 'base64url') {
...
function Qo(e, t) {
	...
	if (
		...
		for (
			i = f.length, a = A.length, v = i < a, v && (a = i), i = 0;
			i < a;
			i++
		)
			if (f[i] != A[i]) {
	...
	Go = fe(() => {
		...
		S.toPrecision = function (e, t) {
			...
			return (
				e === void 0
					? ((r = Z(i)),
						(n = Ye(i, r <= o.toExpNeg || r >= o.toExpPos)))
					: (Re(e, 1, pt),
						t === void 0 ? (t = o.rounding) : Re(t, 0, 8),
						(i = V(new o(i), e, t)),
						(r = Z(i)),
	...
	Ds.exports = (function () {
		...
		return function (t, r) {
			...
			var a = 0,
				...
				ue = [];
			for (f = 0; f < i; f++)
				(ue.push(f + 1), ue.push(t.charCodeAt(s + f)));
...
function ji(e, t, r, n) {
	...
	switch (r.type) {
		...
		case 'Time': {
			...
			throw new W(
				`Expected a time in column '${t}', got ${typeof e}: ${e}`,
			);
		}
		case 'Array':
...
function Sd(e, t = Yl) {
	...
	if (e < 0)
		throw new it(nt.EncodeTimeNegative, `Time must be positive: ${e}`);
	...
	async interpretNode(t, r, n, i) {
		switch (t.type) {
			...
			case 'sum': {
				...
				return {
					value: o.length > 0 ? o.reduce((s, a) => _e(s) + _e(a)) : 0,
				};
			}
			case 'execute': {
				let o = wo(t.args, n, i);
...
async function qd() {
	return (
		globalThis.crypto ?? (await Promise.resolve().then(() => (Ze(), yi)))
	);
}
async function mc() {
		...
		async #r(t) {
			...
			switch (
				...
				case 'waiting':
					return (
						(n.status = 'running'),
						(n.timer = this.startTransactionTimeout(
							n.id,
							r.timeout,
						)),
						{ id: n.id }
				...
				case 'timed_out':
					throw (
						await this.closeTransaction(n, 'timed_out'),
						new Qn()
					);
				case 'running':
				case 'committed':
			...
			try {
				if (t.transaction && r === 'committed')
					if (t.transaction.options.usePhantomQuery)
						await this.#n(Hd(), t.transaction, () =>
							t.transaction.commit(),
						);
					else {
						let n = jd();
						(await this.#n(n, t.transaction, () =>
			...
			(this.startPromise !== void 0 && (await this.startPromise),
				(this.startPromise = (async () => {
					...
					((this.host = r.host),
						...
						(this.remoteClientVersion = await Cc(
							this.host,
							this.config,
						)),
						jr('host', this.host),
